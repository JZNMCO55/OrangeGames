...
## 2026-07-08 傍晚 · 参考图对齐 + Play 2D 机位 + 视觉参数组件化（用户需求驱动，特批跨仓延续）

用户诉求：①史莱姆对齐 `art/refs/slime-12-states-reference.png` ②Play 锁 2D 视角、Edit 还原 3D
③形状/颜色参数可调。三件全落地，真机截图收敛验证：

1. **Play 期 2D 游戏相机 + 暗场（模块所有）**：`SlimeGameModule::OnEnterPlay` 建运行态
   Camera 实体（正交 halfH 1.9 @16:9，对齐 standalone framing）+ Tick 指数平滑跟随 blob
   质心 + Play 期对全部注册 pipeline 关 sky、清屏色 0.012/0.020/0.016（参考图质感靠黑底
   + bloom）。Stop 时实体随快照回收 / 显式销毁，编辑器每帧重申还原 Edit 态背景。
   头文件"相机/背景留宿主"分工注释同步演进。
2. **编辑器 Play 视角锁定（OE ScenePanel）**：Play/Paused 且世界有 Camera 组件时 Scene
   视口让位游戏相机，grid/debug/colliders/sky/清屏重申一并让位；回 Edit 每帧重申自动还原。
   真机验证：Play=2D 黑底特写、Stop=3D 天空网格，无手工步骤。
3. **视觉参数组件化**：SDF 六组颜色从 frag 常量提为 UBO uniform（std140 512→608B，
   宏别名保持使用点原样）；SlimeTuningComponent 增 6 色 + 7 gel 旋钮（serializer 1.0→1.1
   缺字段默认，schema 全标 PlaySafe），模块 Tick 每帧重读 → **Play 中 Inspector/MCP 实时
   调参生效**（MCP set_field allowInPlay 端到端验证）。
4. **参考图收敛**：经 MCP live-tuning 三轮迭代逼近参考图（关键教训：rimGain 1.35 在编辑
   器 bloom 链下被吹成肥大光晕吞掉剪影，降到 ~0.95 + domeScale 1.15→1.38 盖住剪影侧带；
   body/glow 提亮）。最终值烘进 SlimeTuningComponent 与 Tunables 双默认值（保持同步），
   standalone / SlimeEditor / slime.dll 三路径同观感。

已知余项：正交机位 aspect 固定 16:9（视口非 16:9 有轻微拉伸）；机位 halfH/focus 未组件化；
Game 视口在 Edit 态无相机为空（by design，Play 才有游戏相机）。
